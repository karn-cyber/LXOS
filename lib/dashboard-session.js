...
export async function getDashboardSession() {
    ...
    // Auto-provision a Mongo user the first time someone signs in via Clerk.
    // Without this, anything that stores `createdBy`/`submittedBy` (events,
    // reimbursements, achievements) would try to cast the Clerk id ("user_…")
    // to an ObjectId and fail. Upsert keeps it race-safe.
    if (!dbUser) {
        try {
    ...
    const role = dbUser?.role || resolvedRole;
